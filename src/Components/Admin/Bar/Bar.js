...
const Bar = () =>  {
    ...
    const ch1 ="flex items-center h-12 gap-1 mb-4 mt-2 relative rounded-r-xl cursor-pointer "
    const ch2 = "bg-red-200 cursor-pointer transition-all duration-300 ease-in-out"
    return (
        <div className='flex flex-col relative pt-4'>
            ...
            <div className='mt-8 flex flex-col gap-4 mb-4 pl-2' >
                {BarData.map((item, index) => {
                    return (
                        <div className={selected === index ? ch1 + ch2 : ch1}
                                key={index}
                                onClick={()=> setSelected(index)}>
                                    {selected === index ? <div className=' w-2 h-8 bg-red-800 mr-8 ml-2'></div>
                ...
                <div className='flex items-center h-16 gap-2 mt-12 pb-4 pl-4 relative bottom-0' >
                            <FaSignOutAlt />
